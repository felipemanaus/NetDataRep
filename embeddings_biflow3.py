...
# ==============================================================================
# 1. MOTOR MATEMÁTICO (FEATURE EXTRACTOR) - MANTIDO IGUAL
# ==============================================================================
...
# ==============================================================================
# 3. PIPELINE DE EXECUÇÃO (MODIFICADO PARA BIFLOW 5-TUPLE)
# ==============================================================================
...
def run_pipeline(config):
    ...
    print("Cleaning missing IP addresses...")
    # Fill missing IPs with a dummy string and force all to string type
    df[MAP['src_ip']] = df[MAP['src_ip']].fillna("0.0.0.0").astype(str)
    df[MAP['dst_ip']] = df[MAP['dst_ip']].fillna("0.0.0.0").astype(str)
    ...
    # --- MUDANÇA: Normalização para Biflow (Bidirectional 5-Tuple) ---
    print("Normalizando direção dos fluxos (Biflow)...")
    
    # Objetivo: Garantir que A->B e B->A tenham a mesma chave.
    # Regra: Quem tem o 'src_ip' lexicograficamente menor vira o 'node_A'.
    # As portas devem acompanhar os IPs trocados para não quebrar a conexão.
    
    src = df[MAP['src_ip']].values
    dst = df[MAP['dst_ip']].values
    sport = df[MAP['src_port']].values
    dport = df[MAP['dst_port']].values
    
    # Cria uma máscara onde precisamos trocar (quando src > dst)
    # Se src == dst (loopback ou erro), a ordem não importa tanto, mantemos original.
    swap_mask = src > dst
    
    # Vetores normalizados
    node_A_ip = np.where(swap_mask, dst, src)
    node_B_ip = np.where(swap_mask, src, dst)
    node_A_port = np.where(swap_mask, dport, sport)
    node_B_port = np.where(swap_mask, sport, dport)
    
    df['node_A_ip'] = node_A_ip
    df['node_B_ip'] = node_B_ip
    df['node_A_port'] = node_A_port
    df['node_B_port'] = node_B_port
    
    # Chave de Agrupamento Biflow
    key_cols = ['node_A_ip', 'node_B_ip', 'node_A_port', 'node_B_port', MAP['protocol']]
    
    extractor = TrafficFeatureExtractor()
    feat_names = get_feature_names()
    
    print(f"\n🚀 Iniciando Extração (Strategy: Biflow / Bidirectional Flow)")
    print(f"   └── Batch Size: {batch_size}")
    print(f"   └── Agrupamento: 5-Tuple Agnostic to Direction")
    ...
    for name, group in tqdm(groups, desc="Processando Biflows"):
        ...
        if emb_vector is not None:
            ...
            # Metadados do Biflow
            # name é (node_A_ip, node_B_ip, node_A_port, node_B_port, protocol)
            emb_dict['node_A_ip'] = str(name[0])
            emb_dict['node_B_ip'] = str(name[1])
            emb_dict['node_A_port'] = int(float(name[2]))
            emb_dict['node_B_port'] = int(float(name[3]))
            emb_dict['protocol'] = str(name[4])
    ...
    print(f"   └── Biflows Processados: {total_flows}")
    print(f"   └── Biflows Ignorados (<3 pacotes): {skipped_flows}")
...
if __name__ == "__main__":
    ...
    PAINEL_CONFIG = {
        'IO': {
            ...
            'OUTPUT_DIR': f"output_{dataset}_biflow3", 
            'OUTPUT_FILENAME': f"output_{dataset}_embeddings_biflow3.parquet"
        },
        'COLUMNS': {
            'timestamp': 'timestamp',      
            'size': 'packet_length',                
