...
# ==============================================================================
# 3. PIPELINE DE EXECUÇÃO (MODIFICADO PARA SERVICE/DESTINATION PORT)
# ==============================================================================
...
def run_pipeline(config):
    ...
    # --- MUDANÇA: Agrupamento por Serviço (Dst IP, Dst Port, Protocol) ---
    # Ignoramos quem é a origem (src_ip, src_port).
    # O foco é: "Como este serviço está se comportando sob carga de todos?"
    
    key_cols = [MAP['dst_ip'], MAP['dst_port'], MAP['protocol']]
    
    extractor = TrafficFeatureExtractor()
    feat_names = get_feature_names()
    
    print(f"\n🚀 Iniciando Extração (Strategy: Service / Destination Port)")
    print(f"   └── Batch Size: {batch_size}")
    print(f"   └── Agrupamento: {MAP['dst_ip']}:{MAP['dst_port']} ({MAP['protocol']})")
    print(f"   └── Filtro: Ignorando serviços com < 3 pacotes no total")
    ...
    for name, group in tqdm(groups, desc="Processando Serviços"):
        ...
        if emb_vector is not None:
            ...
            # name é a tupla (dst_ip, dst_port, protocol)
            emb_dict['Service_IP'] = str(name[0])
            emb_dict['Service_Port'] = int(float(name[1]))
            emb_dict['Protocol'] = str(name[2])
            
            # Label: Assumimos que se o serviço recebeu tráfego malicioso, ele está sob ataque.
            # Pegamos o primeiro label que aparecer ou majoritário.
            ...
            # Índices de pacotes de VÁRIOS clientes diferentes
            emb_dict['packet_indices'] = group['global_index'].tolist()
    ...
    print(f"   └── Serviços Processados: {total_services}")
    print(f"   └── Serviços Ignorados (<3 pacotes): {skipped_services}")
...
if __name__ == "__main__":
    ...
    PAINEL_CONFIG = {
        'IO': {
            ...
            'OUTPUT_DIR': f"output_{dataset}_service3", 
            'OUTPUT_FILENAME': f"output_{dataset}_embeddings_service3.parquet"
        },
        'COLUMNS': {
            'timestamp': 'timestamp',      
            'size': 'packet_length',                
            'src_ip': 'src_ip', # Usado para leitura, mas ignorado no groupby
            'dst_ip': 'dst_ip',
            'src_port': 'src_port', # Usado para leitura, mas ignorado no groupby
            'dst_port': 'dst_port',
            'protocol': 'protocol',
            'label_multi': 'label',      
